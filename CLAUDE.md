# CLAUDE.md — Expert (Editorial) Website Template
...
**What this is:** The **"expert"** template — a premium, editorial website
template for practitioners of any field (consultants, coaches, advisors,
therapists, studios). It is the magazine-style sibling to the classic/basic
template. Built to be reliably customizable by Claude Code per client.

**Relationship to the classic template:** Expert is a *structural match* to the
classic template — identical file system, identical `global.css` token
architecture, identical `config.yaml` shape, and identical component prop APIs.
The two are a maintainable matched set. **The only thing that differs is the
layout / design language.** When editing, preserve prop APIs so a page written
for one template can be re-skinned by the other.
...
are built fresh to the editorial spec.

**Design language:** Editorial / magazine. Oversized Cormorant serif display
with italic `<em>` accents, asymmetric grids, image/text overlap, sticky side
rails, numbered lists, hairline rules, flat/sharp shapes, generous whitespace.
Where the classic template is soft and centered, expert is structured and
editorial. Full-bleed / overlapping layouts are allowed and encouraged (unlike
the classic template's strict in-grid constraint) — but any section that uses
its own `<section>` tag must still emit `data-section-bg` (see spacing system).

**Must look premium in grayscale.** The design cannot depend on "the right"
colors. Color and font are swappable tokens; the default palette is a neutral
warm cream + espresso register that reads premium before any brand color is
chosen. Keep it that way — do not hard-code colors that only work for one brand.
...
@theme {
  /* Colors — core palette (neutral, grayscale-safe, fully swappable).
     Token NAMES match the classic template; only the VALUES differ. */
  --color-primary: #6c6150;       /* warm taupe — eyebrows, labels, icons, rules, one taupe CTA section */
  --color-primary-dark: #4c4234;  /* darker taupe — secondary button border/text, link hovers */
  --color-secondary: #e8e0d1;     /* pale stone — chips, badge + card backgrounds */
  --color-accent: #33281d;        /* warm espresso — primary buttons, heading italic accents, dark blocks */
  --color-accent-light: #ece3d3;  /* warm paper — soft section backgrounds, card backs */
  --color-accent2: #7c6f57;       /* muted sand — occasional third accent */
  --color-accent2-light: #efe6d6; /* pale sand — panel backgrounds (About) */

  /* Surfaces */
  --color-bg-base: #f7f2e8;       /* warm cream — default section background */
  --color-bg-subtle: #efe7d9;     /* deeper cream — alternating sections */
  --color-bg-card: #fffdf8;

  /* Text */
  --color-text-base: #35302a;
  --color-text-heading: #241d16;
  --color-text-muted: #6f6857;

  /* Borders */
  --color-border: #dcd3c2;
  --color-border-strong: #c2b7a1;

  /* Typography */
  --font-heading: 'Cormorant Variable', Georgia, serif;
  --font-body: 'Outfit Variable', ui-sans-serif, sans-serif;

  /* Radius — flat / sharp for editorial feel (classic uses softer radii) */
  --radius-button: 0.125rem;
  --radius-card: 0.125rem;
  --radius-input: 0.125rem;

  /* Editorial display scale — large; see @layer base for mobile→desktop */
  --text-h1: 4.25rem;
  --text-h2: 2.75rem;
  --text-h3: 1.75rem;
  --text-h4: 1.35rem;
...
Padding values (mobile / desktop) — generous, editorial:
- `tight`: 2.5rem / 3.5rem
- `default`: 4.5rem / 7rem
- `loose`: 6rem / 10rem

`SectionWrapper` also accepts `container?: 'page' | 'wide' | 'narrow'` for editorial
width control (`page` = 80rem default, `wide` = 96rem for magazine bleeds, `narrow`
= 46rem for text columns).

**Rule:** Add `spacing="loose"` to sections that border a strongly contrasting background change (e.g. a cream section before a dark espresso LeadMagnet).

Sections using their own `<section>` tag instead of SectionWrapper (Hero, LeadMagnet, LeadMagnetMini, IsThisYou, FunFacts, TestimonialsSimpleStacked, Marquee, InsuranceBanner, PageHero fullbleed/panels variants) must manually include `data-section-bg={background}` on their `<section>` element.

### Color Philosophy (editorial / neutral)

The palette is deliberately restrained. Rhythm comes from **light cream sections
punctuated by dark espresso blocks** — not from many saturated colors. Because
every value is a token, the whole system re-skins from `@theme {}`.

- **Espresso / ink (`accent`):** Primary buttons, heading italic `<em>` accents,
  and the dark feature blocks (LeadMagnet, IsThisYou, panels PageHero, footer).
  This is the "dark moment."
- **Taupe (`primary`):** Eyebrows, small-caps labels, icons, hairline accents,
  and one reserved dark CTA section (CtaFinal default). A mid-tone, not a pop.
- **Cream (`bg-base`) / deeper cream (`bg-subtle`):** Default section backgrounds,
  alternating for gentle rhythm.
- **Warm paper (`accent-light`) / pale sand (`accent2-light`):** Soft tinted
  sections and panels (Transformation, About panel) — warmth without a color hit.

**Button variants:**
- `primary` — espresso (`accent`) fill, uppercase tracked label, sharp corners
- `secondary` — ink outline (border + text `text-heading`), fills on hover
- `outline` — white outline/text, for use on dark section backgrounds
- `link` / `tertiary` — inline underline action with an arrow (no fill)

**Editorial accent = typography, not color.** The "pop" in this template comes
from the serif italic in headings and the small-caps `.eyebrow` labels — not from
a bright hue. Keep saturated color out; let type and whitespace carry it.

### Color Usage Guide

**Every background token maps to a safe foreground:**

| Background | Token | Foreground options |
|---|---|---|
| Cream / paper / card | `bg-base`, `bg-subtle`, `bg-card` | `text-text-base`, `text-text-heading`, `text-text-muted`, `text-primary` |
| Warm paper / sand panels | `bg-accent-light`, `bg-accent2-light`, `bg-secondary` | `text-text-heading`, `text-text-base`, `text-primary` |
| Espresso (dark block) | `bg-accent` | `text-white`, `text-white/80`, `text-white/60` |
| Taupe (dark block) | `bg-primary` | `text-white`, `text-white/80`, `text-white/60` |

**Rules of thumb:**
- On dark blocks (`accent` / `primary`), text is white at graded opacities; use
  `outline` buttons and `border-white/20` hairlines.
- Every section component exposes `isDark` internally for its dark-background
  variants — follow that pattern when adding variants; never hard-code a light-only color.
- Hairline rules (`border-border`, or `border-white/20` on dark) replace the
  classic template's shadows/rounded cards. Prefer rules over shadows.

### Typography Rules

The editorial voice is carried by **large** Cormorant serif. It is elegant but
renders too light at small sizes.

- **Use `font-heading` big:** headings (h1–h4 apply it automatically), plus
  deliberate large display like pull-quotes, testimonial quotes, and service
  taglines at `text-xl`/`text-2xl` and up. This is the signature look.
- **Never use `font-heading` for body-sized text** (below ~1.4rem). Small copy is
  always the body font (Outfit).

**Italic accent — the editorial signature.** `@layer base` styles `<em>` inside
headings as serif italic. Put emphasis directly in the heading string via
`set:html` (all `heading` props support inline HTML):
```astro
heading="Work that fits the <em>life</em> you actually live."
```
There is **no `headingAccent` prop** — do not add one.

**The `.eyebrow` utility** (defined in `global.css`) is the small-caps, letter-spaced
label used above headings and on links/buttons throughout. Use it for kickers,
labels, and button text rather than re-declaring `uppercase tracking-… text-xs`.

**Reveal on scroll:** add `data-reveal` (and optional `data-reveal-delay={ms}`) to
any element to fade/slide it in. The hidden state is gated behind `html.js`, and a
global observer + failsafe in `BasicScripts.astro` guarantees content is **never**
left hidden (no-JS shows everything; a timeout reveals anything the observer
missed). Do not re-implement per-section observers.
...
@media (min-width: 768px) {
  .container-page { padding-inline: 3rem; }  /* desktop gutter */
}
```
...
- **Don't add dark mode.** Clients don't need it. It adds complexity.
